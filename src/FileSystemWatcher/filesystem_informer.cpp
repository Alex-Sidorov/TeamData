...
QStringList fsw::FileSystemInformer::parse_directory(const QString &path, uint16_t filter)
{
    QStringList data;
    QDirIterator it(path,static_cast<QDir::Filter>(filter), QDirIterator::Subdirectories);
    while (it.hasNext())
    {
        data << it.next();
    }
...
QStringList fsw::FileSystemInformer::get_files(const QString &path)
{
    return parse_directory(path,QDir::Files | QDir::NoDotAndDotDot);
}
