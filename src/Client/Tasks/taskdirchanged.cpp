...
void TasksClient::TaskDirChanged::run_process()
{
    emit complete_task( exec() );
    delete this;
}

bool TasksClient::TaskDirChanged::exec()
{
    ...
    if(dirs != _meta_data->first)
    {
        ...
        WorkMetaDataOnClient::WorkerMetaData::remove_root_path(_root,dirs);
...
QStringList TasksClient::TaskDirChanged::get_removed_files(const QStringList &scan_files, const QString &path)const
{
    QStringList result;
...
QStringList TasksClient::TaskDirChanged::get_added_files(const QStringList &scan_files)const
{
...
QStringList TasksClient::TaskDirChanged::get_removed_folders(const QStringList &scan_dirs, const QString &path)const
{
    QStringList result;
...
QStringList TasksClient::TaskDirChanged::get_added_folders(const QStringList &scan_dirs)const
{
    auto dirs = scan_dirs;
    for(auto &dir : _meta_data->first)
    {
...
void TasksClient::TaskDirChanged::add_dirs_path(QStringList &dirs)
{
    for(auto &dir:dirs)
    {
        auto path = dir.split('/');
...
TasksClient::TaskDirChanged::TaskDirChanged(MDD *meta_data, MDD *present_meta_data, const QString &path, const QString &root, QFileSystemWatcher *watcher, QMutex *mutex):
    _root(root), _path(path), _meta_data(meta_data), _present_meta_data(present_meta_data), _watcher(watcher), _mutex(mutex)
{

}

TasksClient::TaskDirChanged::~TaskDirChanged()
{
}
