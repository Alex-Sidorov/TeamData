...
void TasksClient::TaskFileChanged::run_process()
{
    exec();
    emit complete_task();
    delete this;
...
void TasksClient::TaskFileChanged::exec()
{
...
TasksClient::TaskFileChanged::TaskFileChanged(MDD *meta_data, MDD *present_meta_data, const QString &path, const QString &root, QMutex *mutex):
    _root(root), _path(path), _meta_data(meta_data), _present_meta_data(present_meta_data), _mutex(mutex)
{

}

TasksClient::TaskFileChanged::~TaskFileChanged()
{
}
